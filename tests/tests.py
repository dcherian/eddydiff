...
def test_circle_project_gradient(debug=False):
    ...
    circle = xr.DataArray(
        np.tile(np.exp(-((r / 20) ** 2)), (len(lon), 1, 1)), dims=dims, coords=coords
...
def test_project_vectors():
    ...
    a = xr.DataArray(
        np.reshape(np.arange(nx * ny * nz), [nz, ny, nx]),
        dims=["pres", "lat", "lon"],
        coords={"pres": np.arange(nz), "lat": np.arange(ny), "lon": np.arange(nx)},
    )
    ...
    xr.testing.assert_allclose(ed.project_vector(da, da, "along").mag, da.mag)

    # projection normal to gradient should be zero
    np.testing.assert_almost_equal(
        ed.project_vector(da, da, "normal").mag.values, np.zeros_like(da.dx)
    )
    ...
    xr.testing.assert_allclose(
        ed.project_vector(dx, dy, "normal").mag, xr.ones_like(da.dx)
    )
    xr.testing.assert_allclose(
        ed.project_vector(dx, dy, "along").mag, xr.zeros_like(da.dx)
    )
